...
enum FKEY_ACTIONS {
    ...
    FKEY_GTULTRA_PLAY_FROM_CURRENT_POS,      // Play from current pattern step position OR from current orderlist
                                             // position if currently editing orderlist
    FKEY_GTULTRA_PLAY_FROM_START_OF_PATTERN, // Play from current pattern step position OR from current orderlist
                                             // position if currently editing orderlist
    FKEY_STOP,
    FKEY_MUTE_CHANNEL
};
...
void fkey_set(char* actionList) {
    ...
    if (parameterToSet == FKEY_FOLLOW) followplay = valueToUse;
    else if (parameterToSet == FKEY_LOOP) transportLoopPattern = valueToUse;
}
...
void fkey_do_action(GTOBJECT* gt, char action) {
    if (action == FKEY_PLAY_FROM_START_OF_SONG) orderPlayFromPosition(gt, 0, 0, 0, 1);
        ...
        if (check_for_play_in_order_list(gt) == 0) playFromCurrentPosition(gt, editorInfo.eppos);
    }
    else if (action == FKEY_STOP) {
        stopsong(gt);
        setMasterLoopChannel(gt, "debug_9");
    }
    else if (action == FKEY_MUTE_CHANNEL) mutechannel(editorInfo.epchn, gt);
}

void fkeys_process_action_list(GTOBJECT* gt, char* actionList) {
    while (*actionList != FKEY_DONE) {
        ...
        else if (action == FKEY_TOGGLE) fkey_toggle(actionList);
        else fkey_do_action(gt, action);
    };
}

char key_line_text_buffer[MAX_PATHNAME];

int process_fkey_cfg_text_line(char* str) {
    char  delim[] = " ";
...
int get_fkey_line(FILE* handle, char* textLine) {
    for (;;) {
        if (feof(handle)) return 0;
        if (fgets(textLine, MAX_PATHNAME, handle) == nullptr) return 0;

        if ((textLine[0]) && (textLine[0] != ';') && (textLine[0] != ' ') && (textLine[0] != 13) &&
            (textLine[0] != 10)) {
            ...
            if (l < 0) l = 0;
            for (int i = l; i < e; i++) {
                if (textLine[i] == 10 || textLine[i] == 13) textLine[i] = 0;
            }
            return 1;
        }
    }
...
char F1_OriginalGT[] = {
    FKEY_SET, FKEY_FOLLOW, 0, FKEY_SET, FKEY_LOOP, 0, FKEY_PLAY_FROM_START_OF_SONG, FKEY_DONE
};
char F1_SHIFT_OriginalGT[] = { FKEY_SET, FKEY_FOLLOW, 1, FKEY_SET, FKEY_LOOP, 0, FKEY_PLAY_FROM_START_OF_SONG,
                               FKEY_DONE };

char F2_OriginalGT[]       = { FKEY_SET, FKEY_FOLLOW, 0, FKEY_SET, FKEY_LOOP, 0, FKEY_PLAY_FROM_START_OF_PATTERN,
                               FKEY_DONE };
char F2_SHIFT_OriginalGT[] = { FKEY_SET, FKEY_FOLLOW, 1, FKEY_SET, FKEY_LOOP, 0, FKEY_PLAY_FROM_START_OF_PATTERN,
                               FKEY_DONE };

char F3_OriginalGT[]       = { FKEY_SET, FKEY_FOLLOW, 0, FKEY_SET, FKEY_LOOP, 1, FKEY_PLAY_FROM_START_OF_PATTERN,
                               FKEY_DONE };
char F3_SHIFT_OriginalGT[] = { FKEY_SET, FKEY_FOLLOW, 1, FKEY_SET, FKEY_LOOP, 1, FKEY_PLAY_FROM_START_OF_PATTERN,
                               FKEY_DONE };
...
int fkeys_check(GTOBJECT* gt, int rawkey) {
    ...
    for (int i = 0; i < sizeof(GTFKeyInfo) / sizeof(GTFKEY_ENTRY); i++) {
        ...
        if (key == rawkey) {
            ...
            if (found == 1) {
                fkeys_process_action_list(gt, GTFKeyInfo[i].actionList);
                return 1;
            }
        }
    }
...
int fkeys_loadCFG() {
    ...
    fkeys = fopen(fkeysFilename, "rt");
    if (fkeys) {
        do {
            fkeyTextPtr = &key_line_text_buffer[0];
            if (get_fkey_line(fkeys, fkeyTextPtr) == 1) {
                process_fkey_cfg_text_line(fkeyTextPtr);
            }
            else break;
        } while (1);
        fclose(fkeys);
