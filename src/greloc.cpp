...
char* playeroptname[] = { "Buffered SID-writes",   "Sound effect support",   "Volume change support",
                          "Store author-info",     "Use zeropage ghostregs", "Disable optimization",
                          "zeropage SID playback", "Full SID buffering"
...
#else
#define fliptoscreen() gfx_present()
#endif
...
void relocator(GTOBJECT* gt, int gt2relocMode, int autoSave) {
    ...
    if (gt2relocMode == 0) {
        ...
        if (!autoSave) {
            ...
            if (!strlen(loadedsongfilename)) sprintf(textbuffer, "%s Packer/Relocator", programname);
            else
            ...
            printtext(1,
                      2,
                      getColor(CTITLE, 0),
                      "SELECT PLAYROUTINE OPTIONS: (CURSORS=MOVE/CHANGE, ENTER=ACCEPT, ESC=CANCEL)");
            ...
            while (!selectdone) {
                for (c = 0; c < maxOptions; c++) {
                    ...
                    if (playerversion & (PLAYER_BUFFERED << c)) printtext(24, 3 + c, getColor(color, 0), "Yes");
                    else
                ...
                case KEY_SPACE:
                    ...
                    if (!(playerversion & PLAYER_ZPGHOSTREGS)) playerversion &= ~PLAYER_ZPPLAYSID;
                ...
                case KEY_ESC: selectdone = -1; break;

                case KEY_ENTER: selectdone = 1; break;
                }
            ...
            if (editorInfo.maxSIDChannels > 3) {
                ...
                while (!selectdone) {
                    ...
                    switch (rawkey) {
                    case KEY_LEFT:
                        if (sidAddr2 > 0xd400) sidAddr2 -= 0x0020;
                        break;

                    case KEY_RIGHT:
                        if (sidAddr2 < 0xe000) sidAddr2 += 0x0020;
                        break;
            ...
            if (editorInfo.maxSIDChannels > 6) {
                ...
                while (!selectdone) {
                    ...
                    switch (rawkey) {
                    case KEY_LEFT:
                        if (sidAddr3 > 0xd400) sidAddr3 -= 0x0020;
                        break;


                    case KEY_RIGHT:
                        if (sidAddr3 < 0xe000) sidAddr3 += 0x0020;
                        break;
            ...
            if (editorInfo.maxSIDChannels > 9) {
                ...
                while (!selectdone) {
                    ...
                    switch (rawkey) {
                    case KEY_LEFT:
                        if (sidAddr4 > 0xd400) sidAddr4 -= 0x0020;
                        break;

                    case KEY_RIGHT:
                        if (sidAddr4 < 0xe000) sidAddr4 += 0x0020;
                        break;
            ...
            while (!selectdone) {
                ...
                if (!(playerversion & PLAYER_ZPGHOSTREGS)) {
                    if (zeropageadr < 0x90)
                        sprintf(textbuffer,
                                "$%02X-$%02X (Used by BASIC interpreter)    ",
                                zeropageadr,
                                zeropageadr + 1);
                    if ((zeropageadr >= 0x90) && (zeropageadr < 0xfb))
                        sprintf(textbuffer,
                                "$%02X-$%02X (Used by KERNAL routines)      ",
                                zeropageadr,
                                zeropageadr + 1);
                    ...
                    if (zeropageadr >= 0xfe)
                        sprintf(textbuffer,
                                "$%02X-$%02X ($FF used by BASIC interpreter)",
                                zeropageadr,
                                zeropageadr + 1);
                }
                else {
                    sprintf(textbuffer,
                            "$%02X-$%02X (ghostregs start at %02X)",
                            zeropageadr,
                            zeropageadr + 26,
                            zeropageadr);
                }
                ...
                switch (rawkey) {
                case KEY_LEFT: zeropageadr -= 0x10; break;

                case KEY_UP: zeropageadr++; break;

                case KEY_RIGHT: zeropageadr += 0x10; break;

                case KEY_DOWN: zeropageadr--; break;
    ...
    for (c = 0; c < MAX_PATT; c++) {
        if (pattused[c]) {
            ...
            for (d = 0; d < pattlen[c]; d++) {
                ...
                if (pattern[c][d * 4 + 1]) instrused[pattern[c][d * 4 + 1]] = 1;
    ...
    if (tableerrorcause) {
        ...
        TABLETYPE:
            ...
            case WTBL: strcat(textbuffer, "WAVE"); break;

            case PTBL: strcat(textbuffer, "PULSE"); break;

            case FTBL: strcat(textbuffer, "FILTER"); break;
            }
            strcat(textbuffer, ")");
            break;
        }
    ...
    for (c = 0; c < MAX_TABLES; c++) findtableduplicates(c);
    ...
    do {
        ...
        if (editorInfo.maxSIDChannels > 3) insertdefine("SID2BASE", sidAddr2);
        if (editorInfo.maxSIDChannels > 6) insertdefine("SID3BASE", sidAddr3);
        if (editorInfo.maxSIDChannels > 9) insertdefine("SID4BASE", sidAddr4);
        ...
        for (c = 0; c < MAX_TABLES; c++) {
            ...
            for (d = 0; d < MAX_TABLELEN; d++) {
                if (tableused[c][d + 1]) {
                    ...
                    case PTBL:
                        if ((simplepulse) && (ltable[c][d] != 0xff) && (ltable[c][d] > 0x80)) insertbyte(0x80);
                        else
                    ...
                    default: insertbyte(ltable[c][d]); break;
                    }
                }
            }
                    ...
                    if ((ltable[c][d] != 0xff) || (c == STBL)) {
                        ...
                        case WTBL:
                            if ((ltable[c][d] >= WAVECMD) && (ltable[c][d] <= WAVELASTCMD)) {
                                ...
                                case CMD_PORTADOWN:
                                case CMD_TONEPORTA:
                                case CMD_VIBRATO: insertbyte(tablemap[STBL][rtable[c][d]]); break;

                                case CMD_SETPULSEPTR: insertbyte(tablemap[PTBL][rtable[c][d]]); break;

                                case CMD_SETFILTERPTR: insertbyte(tablemap[FTBL][rtable[c][d]]); break;

                                default: insertbyte(rtable[c][d]); break;
                                }
                            }
                            else {
    ...
    if (!autoSave) {
        ...
        printtext(1,
                  13,
                  getColor(CTITLE, 0),
                  "SELECT FORMAT TO SAVE IN: (CURSORS=MOVE, ENTER=ACCEPT, ESC=CANCEL)");
    ...
    if (!autoSave) {
        if (!songExported) {
            ...
            switch (fileformat) {
            case FORMAT_PRG: strcat(packedsongname, ".prg"); break;

            case FORMAT_BIN: strcat(packedsongname, ".bin"); break;

            case FORMAT_SID: strcat(packedsongname, ".sid"); break;
            }
        }
        ...
        if ((editorInfo.multiplier > 1) || (!editorInfo.multiplier)) {
            ...
            if (editorInfo.multiplier) {
                if (editorInfo.ntsc) speedvalue = 0x42c6 / editorInfo.multiplier;
                    ...
                    speedvalue = 0x4cc7 / editorInfo.multiplier;
            }
            else {
...
PRCLEANUP:
    ...
    if (gt2relocMode == 0 && autoSave == 0) {
        if (oldfileformat != fileformat || oldplayerversion != playerversion || oldzeropageadr != zeropageadr ||
            oldsidAddr1 != sidAddr1 || oldsidAddr2 != sidAddr2 || oldsidAddr3 != sidAddr3 ||
            oldsidAddr4 != sidAddr4) {
            ...
            if (isusedandselfcontained(num, c)) {
                for (d = c + gettablepartlen(num, c - 1); d <= MAX_TABLELEN;) {
                    ...
                    if (isusedandselfcontained(num, d)) {
                        ...
                        if (e == len) {
                            // Duplicate found, remove and map to the original
                            for (e = 0; e < len; e++) tableused[num][d + e] = 0;
                            for (e = d; e < MAX_TABLELEN; e++)
...
int isusedandselfcontained(int num, int start) {
    ...
    for (c = 1; c < start; c++)
        if ((tableused[num][c]) && (ltable[num][c - 1] == 0xff) && (rtable[num][c - 1] >= start) &&
            (rtable[num][c - 1] <= end))
            return 0;
