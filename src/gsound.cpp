...
Sint16* sid0buffer = nullptr;
Sint16* sid1buffer = nullptr;
Sint16* sid2buffer = nullptr;
Sint16* sid3buffer = nullptr;
Sint16* tempbuffer = nullptr;
...
lpWriteToHardSID     WriteToHardSID     = nullptr;
lpReadFromHardSID    ReadFromHardSID    = nullptr;
lpInitHardSID_Mapper InitHardSID_Mapper = nullptr;
lpMuteHardSID_Line   MuteHardSID_Line   = nullptr;
lpHardSID_Delay      HardSID_Delay      = nullptr;
lpHardSID_Write      HardSID_Write      = nullptr;
lpHardSID_Flush      HardSID_Flush      = nullptr;
lpHardSID_SoftFlush  HardSID_SoftFlush  = nullptr;
HINSTANCE            hardsiddll         = 0;
int                  dll_initialized    = FALSE;
// Cycle-exact HardSID support
int          cycleexacthardsid  = FALSE;
SDL_Thread*  playerthread       = nullptr;
SDL_mutex*   flushmutex         = nullptr;
...
void sound_playrout() {
    ...
    if (!bypassPlayRoutine) playroutine(&gtObject);
    if (usehardsid) {
#ifdef __WIN32__
...
void sound_mixer(Sint32* dest, unsigned samples) {
    //	printf("samples: %d\n", samples);
    if (!bypassPlayRoutine) JPSoundMixer(dest, samples);
}
...
int sound_init(unsigned b,
...
#ifdef __WIN32__
    if (!flushmutex) flushmutex = SDL_CreateMutex();
#endif
    ...
    if (hardsid) {
        ...
        if ((righthardsid == lefthardsid) || (righthardsid < 0)) righthardsid = lefthardsid + 1;
        ...
        if (!cycleexacthardsid) {
            ...
            playerthread    = SDL_CreateThread(sound_thread, nullptr, nullptr);
    ...
    if (catweasel) {
#ifdef __WIN32__
        catweaselhandle = CreateFile("\\\\.\\SID6581_1",
                                     GENERIC_READ,
                                     ...
                                     0L);
        if (catweaselhandle == INVALID_HANDLE_VALUE) return 0;
#else
        catweaselfd = open("/dev/sid", O_WRONLY);
        if (catweaselfd < 0) catweaselfd = open("/dev/misc/sid", O_WRONLY);
        if (catweaselfd < 0) return 0;
#ifndef __amigaos__
        if (ntsc) ioctl(catweaselfd, CWSID_IOCTL_NTSC);
        else ioctl(catweaselfd, CWSID_IOCTL_PAL);
#endif
#endif
    ...
    if (writer) writehandle = fopen("sidaudio.raw", "wb");
...
void sound_uninit() {
    ...
    if (usehardsid || usecatweasel) {
        ...
        if (!playerthread) {
            ...
            SDL_WaitThread(playerthread, nullptr);
    ...
    if (sid0buffer) release_mix_buffer(sid0_storage, sid0buffer);
    if (sid1buffer) release_mix_buffer(sid1_storage, sid1buffer);
    if (sid2buffer) release_mix_buffer(sid2_storage, sid2buffer);
    if (sid3buffer) release_mix_buffer(sid3_storage, sid3buffer);
    if (tempbuffer) release_mix_buffer(temp_storage, tempbuffer);
    if (usehardsid) {
#ifdef __WIN32__
        for (c = 0; c < NUMSIDREGS; c++) {
            WriteToHardSID(lefthardsid, c, 0x00);
...
int sound_thread(void* userdata) {
    ...
    while (runplayerthread) {
        ...
        if ((flush_cycles_interactive > 0 && interactive && cycles_after_flush >= flush_cycles_interactive) ||
            ...
            if (HardSID_SoftFlush) HardSID_SoftFlush(lefthardsid);
...
FILE* exportFileHandle    = nullptr;
FILE* exportWAVFileHandle = nullptr;
...
void OpenExportFileNameForWriting() {
    if (exportFileHandle != nullptr) ExportCloseFileHandle();
...
void convertRAWToWAV(int doNormalize) {
    ...
    float normalize = 1;
    if (doNormalize) {
        if (largestExportValue > 0) normalize = 0x7fff / largestExportValue;
    }
...
void ExportSIDToPCMFile(int samples, int doNormalize) {
    ...
    for (int c = 0; c < samples; c++) {
        ...
        if (doNormalize) {
            if (f < 0x8000 && f > largestExportValue) largestExportValue = f;
            else if (f >= 0x8000 && ((0x10000 - f) > largestExportValue)) largestExportValue = (0x10000 - f);
        }
...
void InitHardDLL() {
    ...
    if ((HardSID_Delay) && (HardSID_Write) && (HardSID_Flush) && (HardSID_SoftFlush)) cycleexacthardsid = TRUE;
