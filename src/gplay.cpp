...
uint8_t freqtbllo[] = {  };

uint8_t freqtblhi[] = {  };
...
int       jpd               = 0;
const int release_times[16] = {
    6, 24, 48, 72, 114, 168, 204, 240, 300, 750, 1500, 2400, 3000, 9000, 15000, 24000
};

void update_compressed_seq(int c, CHN* cptr, GTOBJECT* gt) {
    ...
    if (cptr->pattptr >= (pattlen[cptr->pattnum] * 4)) cptr->pattptr = 0;

    // Check for playback endpos
    if ((gt->lastsonginit != PlayMode::Beginning) && (gt->editorUndoInfo.editorInfo[c].esend > 0) &&
        (gt->editorUndoInfo.editorInfo[c].esend > gt->editorUndoInfo.editorInfo[c].espos) &&
        (cptr->songptr > gt->editorUndoInfo.editorInfo[c].esend) &&
        (gt->editorUndoInfo.editorInfo[c].espos < songlen[jnum][c2]))
        cptr->songptr = gt->editorUndoInfo.editorInfo[c].espos;

SEQDONE: {}
}

void update_expanded_seq(int c, CHN* cptr, GTOBJECT* gt) {
    int jnum = getActualSongNumber(gt->psnum, c);
    int c2   = c % MAX_CHN;
    // Song loop
    ...
    */
    ...
    if ((gt->lastsonginit != PlayMode::Beginning) && (gt->editorUndoInfo.editorInfo[c].esend > 0) &&
        (gt->editorUndoInfo.editorInfo[c].esend > gt->editorUndoInfo.editorInfo[c].espos) &&
        (cptr->songptr > gt->editorUndoInfo.editorInfo[c].esend) &&
        (gt->editorUndoInfo.editorInfo[c].espos < songOrderLength[jnum][c2]))
        cptr->songptr = gt->editorUndoInfo.editorInfo[c].espos;
...
void sequencer(int c, CHN* cptr, GTOBJECT* gt) {
    if (c >= editorInfo.maxSIDChannels) return;

    if ((gt->songinit != PlayMode::Stopped) && (cptr->pattptr == 0x7fffffff)) {
        cptr->pattptr = gt->startpattpos * 4;

        if (!cptr->advance) {
            if (!transportLoopPattern) // V1.3.6 - in PATTERN_PLAY mode, only loop if loop is enabled in the
                                       // transport bar
                gt->songinit = PlayMode::Stop;
            goto SEQDONE;
        }

        if (editorInfo.expandOrderListView == 0) update_compressed_seq(c, cptr, gt);
        else update_expanded_seq(c, cptr, gt);
    }
SEQDONE: {}
}
...
void initchannels(GTOBJECT* gt) {
    ...
    for (c = 0; c < MAX_PLAY_CH; c++) {
        gt->chn[c].trans = 0;
        gt->chn[c].instr = 1;
        if (editorInfo.multiplier) cptr->tempo = 6 * editorInfo.multiplier - 1;
        else cptr->tempo = 6 - 1;
        cptr++;
    }
...
void initsongpos(int num, PlayMode mode, int pattpos, GTOBJECT* gt) {
    gt->loopEnabledFlag = 0;
...
void rewindsong(GTOBJECT* gt) {
    if (gt->lastsonginit == PlayMode::Beginning) gt->lastsonginit = PlayMode::Pos;
...
bool isplaying(GTOBJECT* gt) { return gt->songinit != PlayMode::Stopped; }
...
void playroutine(GTOBJECT* gt) {
    ...
    if (static_cast<int>(gt->songinit) > 0 && gt->songinit < PlayMode::Stopped) {
        ...
        if (gt->songinit == PlayMode::Pos || gt->songinit == PlayMode::Pattern) {
            for (c = 0; c < editorInfo.maxSIDChannels; c++) {
                ...
                if (editorInfo.expandOrderListView == 0) {
                    if (gt->editorUndoInfo.editorInfo[c2].espos >= songlen[songNum][c3])
                        gt->songinit = PlayMode::Beginning;
                }
                else {
        ...
        for (c = 0; c < MAX_PLAY_CH; c++) {
            ...
            cptr->newnote              = 0;
            cptr->repeat               = 0;
            if (editorInfo.multiplier) cptr->tick = 6 * editorInfo.multiplier - 1;
            else cptr->tick = 6 - 1;
        ...
        if (gt->songinit != PlayMode::Stop) gt->songinit = PlayMode::Playing;
        else gt->songinit = PlayMode::Stopped;
        ...
        for (int i = 0; i < editorInfo.maxSIDChannels; i++) {
            ...
            if (editorInfo.expandOrderListView == 0) {
                if (!songlen[baseSong + j][k]) {
                    gt->songinit = PlayMode::Stopped; // Zero length song
                    break;
                }
            }
            else {
        ...
        for (c = 0; c < MAX_PLAY_CH; c++) {
            ...
            if ((gt->songinit == PlayMode::Stopped && cptr->tempo < 2) ||
                c >= editorInfo.maxSIDChannels) // JP FEB 17 added maxSIDChannel check
            {
                if (editorInfo.multiplier) cptr->tempo = 6 * editorInfo.multiplier - 1;
                else cptr->tempo = 6 - 1;
            }
            ...
            {
                if (cptr->tempo >= 2) cptr->tick = cptr->tempo;
                else {
        ...
        TICK0:
            ...
            if (cptr->newnote) {
                ...
                if (cptr->newcommand != CMD_TONEPORTA) {
                    ...
                    if (cptr->ptr[WTBL]) {
                        ...
                        if (ltable[WTBL][cptr->ptr[WTBL] - 1] == 0xff) stopsong(gt);
                    }
                    if (iptr->ptr[PTBL]) {
                        ...
                        if (cptr->ptr[PTBL]) {
                            // Stop the song in case of jumping into a jump
                            if (ltable[PTBL][cptr->ptr[PTBL] - 1] == 0xff) stopsong(gt);
                        }
                    }
                    if (iptr->ptr[FTBL]) {
                        ...
                        if (gt->filterInfo[sidIndex].filterptr) {
                            // Stop the song in case of jumping into a jump
                            if (ltable[FTBL][gt->filterInfo[sidIndex].filterptr - 1] == 0xff) stopsong(gt);
                        }
                    }
                    if (!gt->noSIDWrites) {
                        ...
                        if (stereoMode == 2) // TrueStereo panning enabled
                        {
                            ...
                            if (panMax > panMin) cptr->pan = panMin += rand() % (panMax - panMin);
                            else if (panMin > panMax) cptr->pan = panMax += rand() % (panMin - panMax);
                            else cptr->pan = panMin;
                        }
                    ...
                    cptr->releaseTime = release_times[rIndex];
                }
            }
            ...
            case CMD_SETWAVE: cptr->wave = cptr->newcmddata; break;
            ...
            case CMD_SETFILTERCUTOFF: gt->filterInfo[sidIndex].filtercutoff = cptr->newcmddata; break;

            case CMD_SETMASTERVOL:
                if (cptr->newcmddata < 0x10) gt->masterfader = cptr->newcmddata;
                break;
            ...
            case CMD_FUNKTEMPO:
                ...
                {
                    int d;
                    for (d = 0; d < editorInfo.maxSIDChannels; d++) gt->chn[d].tempo = 0;
                }
                break;
            ...
            case CMD_SETTEMPO: {
                ...
                if (cptr->newcmddata >= 0x80) cptr->tempo = newtempo;
                else {
                    int d;
                    for (d = 0; d < editorInfo.maxSIDChannels; d++) gt->chn[d].tempo = newtempo;
                }
            } break;
            }
            if (cptr->newnote) {
            ...
            if (cptr->ptr[WTBL]) {
                ...
                if (wave > WAVELASTDELAY) {
                    ...
                    if ((wave >= WAVECMD) && (wave <= WAVELASTCMD)) {
                        ...
                        case CMD_SETWAVEPTR:
                        case CMD_FUNKTEMPO: stopsong(gt); break;
                        ...
                        case CMD_VIBRATO: {
                            ...
                            if ((cptr->vibtime < 0x80) && (cptr->vibtime > cmpvalue)) cptr->vibtime ^= 0xff;
                            cptr->vibtime += 0x02;
                            if (cptr->vibtime & 0x01) cptr->freq -= speed;
                        ...
                        case CMD_SETWAVE: cptr->wave = param; break;
                        ...
                        case CMD_SETFILTERCUTOFF: gt->filterInfo[sidIndex].filtercutoff = param; break;

                        case CMD_SETMASTERVOL:
                            if (param < 0x10) gt->masterfader = param;
                            break;
                        }
                    }
                }
                ...
                if (note != 0x80) {
                    if (note < 0x80) note += cptr->note; // relative offset (arp chord)
                    note &= 0x7f;
        ...
        PULSEEXEC:
            if (editorInfo.optimizepulse) {
                if (c < editorInfo.maxSIDChannels) {
                    if ((gt->songinit != PlayMode::Stopped) && (cptr->tick == cptr->gatetimer)) goto GETNEWNOTES;
                }
            }
            ...
            if ((gt->songinit == PlayMode::Stopped) || (cptr->tick != cptr->gatetimer) ||
                (c >= editorInfo.maxSIDChannels))
                goto NEXTCHN;
        ...
        GETNEWNOTES: {
            ...
            if (pattern[cptr->pattnum][cptr->pattptr + 1]) cptr->instr = pattern[cptr->pattnum][cptr->pattptr + 1];
            cptr->newcommand = pattern[cptr->pattnum][cptr->pattptr + 2];
            cptr->newcmddata = pattern[cptr->pattnum][cptr->pattptr + 3];

            cptr->pattptr += 4;

            if (pattern[cptr->pattnum][cptr->pattptr] == ENDPATT) cptr->pattptr = 0x7fffffff;

            if (SIDTracker64ForIPadIsAmazing == 0) {
                if (newnote == KEYOFF) cptr->gate = 0xfe;
            }
            else if (newnote == KEYOFF || newnote == REST) // JP added REST
                cptr->gate = 0xfe;

            if (newnote == KEYON) cptr->gate = 0xff;
            if (newnote <= LASTNOTE) {
        ...
        NEXTCHN:
            ...
            if (cptr->mute) {
                ...
                if (!gt->noSIDWrites) {
                    ...
                    if (cptr->releaseTime < 0) cptr->releaseTime = 0;
                }
            }
            cptr++;
        }
    ...
    if (gt->noSIDWrites == 0 && gt->loopEnabledFlag && gt->disableLoopSearch == 0 &&
        ...
        // Check to see if playback has reached the end of the pattern that is being tracked by the Master Loop
        // Channel If so - and if looping is enabled, reset all channels to their correct positions. This syncs,
        // taking into consideration that patterns can be different lengths, or have different tempos, etc.

        if (gt->chn[gt->masterLoopChannel].advance) // V1.3.6
        {
            for (int i = 0; i < editorInfo.maxSIDChannels; i++) {
                if (gt->chn[i].pattptr == gt->loopEndChn[i].pattptr &&
                    gt->chn[i].songptr == gt->loopEndChn[i].songptr && gt->loopEndChn[i].tick == gt->chn[i].tick) {
                    // Only copy specific parts, as we don't want to reset instrument info when looping (so, no
                    // blanket memcpy)
    ...
    if (gt->noSIDWrites == 0 &&
        gt->disableLoopSearch ==
            0) // only the PLAYING GTObject should check for looping. Otherwise, we end up in recursive hell
    {
        if (transportLoopPattern && transportLoopPatternSelectArea) {
            if (gt->chn[gt->masterLoopChannel].advance) // V1.3.6
            {
                ...
                if (markStart != markEnd &&
                    editorInfo.epmarkchn >=
                        0) // 1.1.7 FIX (only check for inter-pattern looping if area is selected) 3/5/2022
                {
                    ...
                    if (gt->chn[editorInfo.epmarkchn].pattptr == markEnd * 4) {
                        //					if (calcStartofInterPatternLoop(gt->psnum, c3, gt->chn[c3].songptr - 1,
                        //&gtLoopObject))
                        if (calcStartofInterPatternLoop(gt->psnum,
                                                        c3,
                                                        gt->chn[editorInfo.epmarkchn].songptr - 1,
                                                        &gtLoopObject)) {
                            gtObject.interPatternLoopEnabledFlag =
                                (markEnd) + ((editorInfo.epmarkchn + 1)
                                             << 8); // c3+1 - just to make sure we don't get a 0 flag value
                        }
                    }
                }
            }
        ...
        if (gt->chn[gt->masterLoopChannel].pattptr ==
            ...
            if (gt->chn[gt->masterLoopChannel]
                    .advance) // V 1.3.6 stops lockup when playing in PlayMode::Pattern mode
            {
                gt->chn[gt->masterLoopChannel].songLoopPtr = gt->chn[gt->masterLoopChannel].songptr;

                //		sprintf(textbuffer, "end song %x mlc %x ec %x em %x %d",
                // gt->chn[gt->masterLoopChannel].songLoopPtr, gt->masterLoopChannel, editorInfo.epchn,
                // editorInfo.editmode, jpd++);
                calculateLoopInfo2(gt->psnum,
                                   gt->masterLoopChannel,
                                   gt->chn[gt->masterLoopChannel].songptr - 1,
                                   &gtLoopObject);
