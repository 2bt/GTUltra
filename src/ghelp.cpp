...
constexpr std::string_view kPattCmds[] = {
    ...
    "Command 3XY: Toneportamento. Raise or lower pitch until target note has been reached. XY is index to a 16-bit speed or 00 for \"tie note\".",
    "Command 4XY: Vibrato. XY is index to speedtable. Left side value determines how long until the direction changes (speed) and right side value is the amount of pitch change each tick (depth).",
    "Command 5XY: Set attack/decay register to value XY.",
    "Command 6XY: Set sustain/release register to value XY.",
    "Command 7XY: Set waveform register to value XY. If a wavetable is actively changing the channel's waveform at the same time, will be ineffective.",
    ...
    "Command BXY: Set filter control. X is resonance and Y is channel bitmask. 00 turns filter off and also stops filtertable execution.",
    "Command CXY: Set filter cutoff to XY. Can be ineffective if the filtertable is active and also changing the cutoff.",
    "Command DXY: Set mastervolume to Y, if X is 0. If X is not 0, value XY is copied to the timing mark location, which is playeraddress+$3F.",
    "Command EXY: Funktempo. XY is an index to speedtable. Will alternate left side and right side tempo values on each pattern step.",
    "Command FXY: Set tempo. Values 03-7F set tempo on all channels, values 83-FF only on current channel (subtract 80 to get actual tempo). Tempos 00 and 01 recall the funktempos set by EXY command.",
};
...
constexpr std::string_view kTableEncoding[] = {
    "Wavetable left side: 00 leave waveform unchanged; 01-0F delay 1-15 frames; 10-DF waveform values; E0-EF inaudible waveform 00-0F; F0-FE execute command 0XY-EXY (right side = parameter); FF jump (right side = position, 00 = stop).",
    "Wavetable right side: 00-5F relative notes; 60-7F negative relative notes; 80 keep frequency unchanged; 81-DF absolute notes C#0 - B-7.",
    "Pulsetable left side: 01-7F pulse modulation step (left = time, right = signed 8-bit speed); 8X-FX set pulse width (X = high 4 bits, right = low 8 bits); FF jump (00 = stop).",
    "Filtertable left side: 00 set cutoff (right side); 01-7F filter modulation (left = time, right = signed speed); 80-F0 set filter parameters (high nybble = passband, right = resonance/channel bitmask as in BXY); FF jump (00 = stop).",
    "Speedtable vibrato: XX YY — left = ticks until direction change (speed), right = pitch delta each tick (depth).",
    ...
    "For vibrato and portamento, if XX has the high bit ($80) set, note-independent depth/speed calculation is enabled and YY is the divisor (higher → quieter effect, more raster time).",
};
...
void print_reference(std::ostream& out)
{
    for (const Topic& t : topics()) {
        ...
        out << t.title << '\n';
        for (std::string_view line : t.body)
            out << "  " << line << '\n';
        out << '\n';
    }
}
