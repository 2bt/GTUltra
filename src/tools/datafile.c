...
int main(int argc, char **argv)
{
	...
	if (argc < 3)
	{
		printf("Usage: DATAFILE <datafile> <filelistfile>\n\n"
		       "The purpose of this program is to gather many files into one datafile.\n"
		       "The filelistfile must contain the name (use only 8+3 chars) of each file\n"
		       "on its own row.\n");
		return 0;
	}
	...
	for (;;)
	{
		...
		if (fscanf(listfile, "%63s", searchname) == EOF)
			break;
		...
		if (test)
		{
			...
			for (c = (int)strlen(fullname[files]) - 1; c >= 0; c--)
			{
				if (fullname[files][c] == '\\')
				{
					c++;
			...
			{
				header[files].name[d] = (char)toupper((unsigned char)fullname[files][c]);
				c++;
				d++;
			}
			files++;
	...
	fwritele32(datafile, (unsigned)files);
	for (c = 0; c < files; c++)
	{
...
int addfile(HEADER *header, FILE *dest, char *name)
{
	...
	header->offset = (uint32_t)ftell(dest);
	fseek(src, 0, SEEK_END);
	header->length = (uint32_t)ftell(src);
	...
	fclose(src);
	printf("* Writing file (size was %u)\n", header->length);
