    ...
    */
    ...
    for (c = 3; c < argc; c++) {
...
#endif
        {
            ...
            case 'A': sscanf(&argv[c][2], "%x", &adparam); break;

            case 'G': sscanf(&argv[c][2], "%f", &basepitch); break;

            case 'L': sscanf(&argv[c][2], "%x", &sidaddress); break;

            case 'O': sscanf(&argv[c][2], "%u", &optimizepulse); break;

            case 'R': sscanf(&argv[c][2], "%u", &optimizerealtime); break;

            case 'V': sscanf(&argv[c][2], "%u", &finevibrato); break;

            case 'S': sscanf(&argv[c][2], "%u", &multiplier); break;

            case 'T': sscanf(&argv[c][2], "%x", &SIDTracker64ForIPadIsAmazing); break;
            ...
            case 'Z': sscanf(&argv[c][2], "%x", &zeropageadr); break;
            }
        }
        else {
