...
void undoInitAllAreas(GTOBJECT* gt) {
    ...
    // 210
    //	undoInitUndoArea((char*)&gt->editorInfo, sizeof(CHN_EDITOR_INFO)*MAX_PLAY_CH,
    ...
    undoInitUndoArea((char*)&songOrderLength,
                     MAX_SONGS * MAX_CHN * sizeof(int),
                     UNDO_AREA_ORDERLIST_LENGTH_EXPANDED,
                     0);

    for (int s = 0; s < MAX_SONGS; s++) {
        for (int i = 0; i < MAX_CHN; i++) {
            undoInitUndoArea((char*)&songOrderPatterns[s][i],
                             MAX_SONGLEN_EXPANDED * sizeof(char),
                             UNDO_AREA_ORDERLIST_PATTERN_EXPANDED,
                             i + (s * MAX_CHN));
            undoInitUndoArea((char*)&songOrderTranspose[s][i],
                             MAX_SONGLEN_EXPANDED * sizeof(short),
...
int undoInvalidateBuffer(int areaType) {
    ...
    if (!initUndoBufferFlag) return 0;
...
int undoValidateUndoAreas(GTUNDO_OBJECT* editorSettings) {
    ...
    for (int i = 0; i < UNDO_AREA_SIZE; i++) {
        ...
        if (gArea != nullptr) {
            ...
            {
                if (debugEnabled) {
                    if (gArea->undoAreaType != UNDO_AREA_CHANNEL_EDITOR_INFO) // don't show false positives where
                                                                              // the editor info differs... for now
                    {
                        int diff =
                            memcmp(gArea->undoObject->dest, gArea->undoObject->data, gArea->undoObject->size);
                        if (diff) {
                            fprintf(stderr,
                                    "%d undo mismatch: i:%d a:%d s:%d\n",
                                    memLeakCount,
                                    i,
                                    gArea->undoAreaType,
                                    gArea->undoAreaSubIndex);
                            memLeakCount++;
                        }
                    }
                }
                ...
                if (diff) {
                    ...
                    while (currentOffset < gArea->undoObject->size) {
                        if (getUndoPacket(&currentOffset, cmp1, cmp2, &startOffset, &endOffset, size)) {
                            ...
                            undoAddUndoObjectToList(gArea->undoObject,
                                                    gArea,
                                                    gArea->undoObject->data,
                                                    sizeToUndo,
                                                    startOffset);
                            ...
                            // Mark as dirty. So we know to finalize the packets (so they all process with a single
                            // ctrl-z)
                            areaDirty++;
                        }
                        else
    ...
    if (areaDirty) {
        // mark the packets so that they all update with ctrl-z is pressed. Including editor update so cursor is in
        // correct pos, etc.
    ...
    while (co < areaSize) {
        ...
        if (a != b) {
            ...
            *endOffset = co;
            ...
            waitForPacketCounter++; // check to see if there's any more changes around this area (saves on data
                                    // packets if we can catch as much as possible
...
int quickAddObjectToList(char* m, GTUNDO_AREA* gArea) {
    ...
    if (gArea != nullptr) uo->type = gArea->undoAreaType;
    else
        uo->type = 0x4567;
...
void undoFinalizeUndoPackage(GTUNDO_OBJECT* editorSettings) {
    ...
    for (int i = 0; i < undoPackageCounter; i++) {
        ...
        ed->counter =
            (undoPackageCounter - 1) - i; // used to know how many packages to process when ctrl-z is pressed
    }
...
void undoAddEditorSettingsToList() {
    if (memcmp(&editorInfo, undoEditorInfoBackup->data, sizeof(EDITOR_INFO))) {
        ...
        quickAddObjectToList((char*)undoEditorInfoBackup, nullptr);
        undoPackageCounter = 0;
    }
    else {
...
int undoPerform(GTOBJECT* gt) {
    ...
    if (currentUndoPosition == 0) return 0;
    ...
    do {
        ...
        if (gu->parentArea != nullptr) // nullptr if overwriting editor UI
        {
